...
DrawMenu::DrawMenu(sf::RenderWindow& window) {\
	...
	//vector <string> texts = { "play", "exit" };
	////vector <sf::Text::setFillColor()> colors = { sf::Color::Red, sf::Color::White, sf::Color::White };
	//for (int i = 0; i < MAX_ITEM_NUMBER; ++i) {
	//	text[i].setFont(font);
	//	text[i].setString(texts[i]);
	//	text[i].setPosition(sf::Vector2f(width / 2, height / (MAX_ITEM_NUMBER + 1) * (i + 1)));
	//}
}
